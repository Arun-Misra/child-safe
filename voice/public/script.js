...
async function startMic() {
    try {
        ...
        workletNode.port.onmessage = (e) => {
            ...
            // ⚡ THE FIX: Catch the instant local trigger and kill the speakers
            if (typeof e.data === 'string' && e.data === 'LOCAL_BARGE_IN') {
                if (isPlaying) {
                    console.log('⚡ Local Barge-In! Instantly muting AI.');
                    stopPlayback(); 
                    loadingIndicator.classList.add('hidden');
                }
                return;
            ...
            // Otherwise, send the raw audio buffer to Gemini
